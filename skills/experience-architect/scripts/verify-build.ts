...
/**
 * Build Verification Utility (Bun)
 ...
 *
 * Usage:
 *   bun verify-build.ts [--src <dir>] [--dist <dir>] [--budget 50] [--max-islands 3] [--skip-build]
 *
...
const USAGE = 'Usage: bun verify-build.ts [--src <dir>] [--dist <dir>] [--budget 50] [--max-islands 3] [--skip-build]';

function parseArgs(argv: string[]) {
  const opts: { src: string; dist: string; budget: number; maxIslands: number; skipBuild: boolean } = {
    src: 'src', dist: 'dist', budget: 50, maxIslands: 3, skipBuild: false,
  };
...
function main() {
  ...
  const report: { passed: boolean; checks: Check[] } = { passed: true, checks: [] };
  ...
  if (jsFiles.length === 0) {
    ...
    report.checks.push({ name: 'largest_bundle', ok: true, detail: `${path.basename(largest.f)} = ${kb(largest.s)} KiB` });
  }
...
function finish(report: { passed: boolean; checks: Check[] }) {
  console.log('\n--- Build Verification Report (Bun) ---');
  for (const c of report.checks) {
    console.log(`${c.ok ? 'PASS' : 'FAIL'}  ${c.name}: ${c.detail}`);
  }
  console.log('---------------------------------------');
  console.log(report.passed
    ? 'Status: All checks passed. Record this in experience_state.json -> build_verification.'
    : 'Status: FAILED — fix the items above before handing off to the Automation Architect.');
